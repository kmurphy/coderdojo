...
fill_color = None
for k in range(2):
    draw_triangle(0, height/3, 2*width/6, height/6, color, fill_color)
    draw_triangle(0, height/4, 2*width/4, height/4, color, fill_color)
    draw_triangle(0, height/8, 2*width/3, height/3, color, fill_color)
    color = None
    fill_color = 'forest green'

# draw ground
bob.pensize(1)
draw_rectangle(-width/2, -height/2, width, height/6, 'white', 'white')
